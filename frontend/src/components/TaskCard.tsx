...
export function TaskCard({
  ...
  // Pasar el ID de la tarea para saber si ESTA tarea específica está en loading
  const { isDeletingTask, isMovingTask } = useLoadingStates(task._id);
  ...
  const textColorClass =
    textContrast === "light" ? "text-white" : "text-gray-700";
  const mutedTextClass =
    textContrast === "light" ? "text-gray-100" : "text-muted-foreground";
  ...
  return (
    <Card
      ...
      className={`mb-2 cursor-grab active:cursor-grabbing hover:shadow-md transition-all relative ${
        isDragging
          ? "shadow-2xl ring-2 ring-primary scale-105 opacity-80 rotate-2"
          : ""
      } ${task.color ? "bg-opacity-10" : ""} ${
        (isDeletingTask || isMovingTask) ? "opacity-50" : ""
      }`}
      {...attributes}
      {...listeners}
    >
      {(isDeletingTask || isMovingTask) && (
        <div
          className="absolute inset-0 bg-background/80 rounded-lg flex items-center justify-center z-50"
          onPointerDown={(e) => e.stopPropagation()}
        >
          <div className="flex flex-col items-center gap-2">
            <LoadingSpinner size="md" />
            <span className="text-xs text-muted-foreground">
              {isDeletingTask ? "Eliminando..." : "Moviendo a otra columna..."}
            </span>
          </div>
        </div>
      )}
            ...
            >
              <PopoverTrigger asChild>
                <Button
                  ...
                  size="icon"
                  className={`h-7 w-7 ${
                    textContrast === "light" ? "hover:bg-white/20" : ""
                  }`}
                >
