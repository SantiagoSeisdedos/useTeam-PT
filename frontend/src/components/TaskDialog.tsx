...
export function TaskDialog({
  ...
  const [improvedDescription, setImprovedDescription] = useState<string | null>(
    null
  );
  ...
  useEffect(() => {
    const fetchAIStatus = async () => {
      try {
        ...
        const currentModelProvider = aiModel.startsWith("gpt")
          ? "openai"
          : "gemini";
        ...
        if (!isCurrentAvailable) {
          ...
          if (status.openai.available && status.openai.models.length > 0) {
            ...
            status.gemini.available &&
            status.gemini.models.length > 0
          ) {
            setAiModel(status.gemini.models[0] as AIModel);
          }
        }
      } catch {
        toast.error("Error al obtener el estado de IA");
      }
    };
    ...
    try {
      ...
      toast.success("Descripción mejorada con IA", {
        description: `Modelo: ${result.model} (${
          result.mode === "simple" ? "Simple" : "Con contexto"
        })`,
      });
    } catch (error) {
      toast.dismiss();
                    ...
                    !aiStatus.gemini.available && (
                      <div className="p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg">
                        <p className="text-xs text-yellow-800 dark:text-yellow-200">
                          ⚠️ No hay proveedores de IA configurados. Configura
                          OPENAI_API_KEY o GEMINI_API_KEY en el backend.
                        </p>
                      </div>
                    )}
                    ...
                    >
                      ...
                      <div className="space-y-2">
                        <Label className="text-xs font-semibold text-muted-foreground">
                          OpenAI{" "}
                          {!aiStatus?.openai.available && "(No disponible)"}
                        </Label>
                        <div className="flex items-center space-x-2">
                          <RadioGroupItem
                            value="gpt-4o-mini"
                      ...
                      <div className="space-y-2 pt-2">
                        <Label className="text-xs font-semibold text-muted-foreground">
                          Google Gemini{" "}
                          {!aiStatus?.gemini.available && "(No disponible)"}
                        </Label>
                        <div className="flex items-center space-x-2">
                          <RadioGroupItem
